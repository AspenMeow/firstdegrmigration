...
setwd("H:/GitHub/mjrmigrationchord/major migration first to degree dev/json")
...
for (y in firstcoll){
        ...
        for (j in c('COLLEGE_FIRST_NAME','DEPT_FIRST_NAME','MAJOR_NAME_FIRST')){
                for (k in c('COLLEGE_DEGREE_NAME','DEPT_DEGREE_NAME','MAJOR_NAME_DEGREE')){
                        AG1 <- data %>% group_by_('COHORT',j,k)%>% summarise(Headcount=n())
                       # AG1$degtype <- substr( k,1,regexpr('_',k)-1)
                        ...
                        names(AG1)[names(AG1) == 'COHORT'] <- 'Entering_Cohort'
                        write.xlsx2( as.data.frame(AG1),
                                    file = paste0("H:/GitHub/mjrmigrationchord/major migration first to degree dev/json/csv/data",y,".xlsx"), 
                                    sheetName = paste0(substr( j,1,regexpr('_',j)-1),'-to-',substr( k,1,regexpr('_',k)-1)), append = T,
                                    showNA="", row.names = F)
                }
        }
